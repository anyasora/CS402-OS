...
pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
	...
	if(result == NULL)
	{
		dbg( DBG_PRINT, "\n GRADING3D INSIDE pframe_get : result = null. EXITING \n");
		return 0;
	}
	/*If the page is already present, then call the pframe_get_resident and get the page.
	 * It may be busy, so we need to keep waiting.
	 */

	*result = pframe_get_resident(o,pagenum);
	if(*result != NULL)
	{
		dbg( DBG_PRINT, "\n GRADING3D INSIDE pframe_get : result is not null \n");

		while(pframe_is_busy(*result))
		{
			dbg( DBG_PRINT, "\n GRADING3D INSIDE pframe_get : pframe is busy \n");
			sched_sleep_on( &(*result)->pf_waitq );
			*result = pframe_get_resident(o, pagenum);
		}
	}
	/*
	 *If it is not able to find the pagenum, then we need to get the page from Disk
	 *get a pagefram and assign the page to the pageframe. We need to first find the
	 *free pageframe.
	 */
	else
	{
		dbg( DBG_PRINT, "\n GRADING3D INSIDE pframe_get : result is null \n");
		*result = pframe_alloc(o, pagenum);
		...
		stat = pframe_fill(*result);
		if(stat < 0)
		{
			dbg( DBG_PRINT, "\n GRADING3D INSIDE pframe_get : stat is not 0. Return stat\n");
			/*Need to write DBG statements.*/
            pframe_free(*result);
			return stat;
		}
		...
		{
			dbg( DBG_PRINT, "\n GRADING3D INSIDE pframe_get : page_free_count == nfreepages_target \n");
			pframe_pin(*result);
			pageoutd_wakeup();
	...
	KASSERT(NULL != *result);
	dbg(DBG_PRINT,"GRADING3A 1.a KASSERT(NULL != *result); SUCCESSFUL \n");

	KASSERT(!pframe_is_busy(*result));
	dbg(DBG_PRINT,"GRADING3A 1.a KASSERT(!pframe_is_busy(*result)); SUCCESSFUL \n");

	dbg( DBG_PRINT, "\n GRADING3D EXITING pframe_get \n");
	return 0;
}
...
pframe_pin(pframe_t *pf)
{
	...
	dbg( DBG_PRINT, "\n GRADING3D ENTERING pframe_pin \n");

	KASSERT(!pframe_is_free(pf));
	dbg(DBG_PRINT,"GRADING3A 1.b KASSERT(!pframe_is_free(pf)); SUCCESSFUL \n");

	KASSERT(pf->pf_pincount >= 0);
	dbg(DBG_PRINT,"GRADING3A 1.b KASSERT(pf->pf_pincount >= 0); SUCCESSFUL \n");

	
	if ((!(pframe_is_pinned(pf))) && (pf->pf_pincount > 0))
	{
		dbg( DBG_PRINT, "\n GRADING3D INSIDE pframe_pin : pf_pincount != 0 \n");
		list_remove(&pf->pf_link);
...
pframe_unpin(pframe_t *pf)
{
	...
	dbg( DBG_PRINT, "\n GRADING3D ENTERING pframe_unpin \n");

	KASSERT(!pframe_is_free(pf));
	dbg(DBG_PRINT,"GRADING3A 1.c KASSERT(!pframe_is_free(pf)); SUCCESSFUL \n");

	KASSERT(pf->pf_pincount > 0);
	dbg(DBG_PRINT,"GRADING3A 1.c KASSERT(pf->pf_pincount > 0); SUCCESSFUL \n");

	pf->pf_pincount--;
	if ((!(pframe_is_pinned(pf))) && (pf->pf_pincount > 0))
	{
		dbg( DBG_PRINT, "\n GRADING3D INSIDE pframe_unpin : pf_pincount != 0 \n");
		list_remove(&pf->pf_link);
		list_insert_tail(&alloc_list,&pf->pf_link);
		npinned--;
		nallocated++;
	}
	dbg( DBG_PRINT, "\n GRADING3D EXITING pframe_unpin \n");
}
