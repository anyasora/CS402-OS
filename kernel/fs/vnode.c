...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
	...
	return 0;
	Read(bytedev_t *dev,int offset,void *buff,int count);
	*/
	int read_count;
	KASSERT(file);
	dbg(DBG_PRINT,"(GRADING2A 1.a) KASSERT(file) is successful\n");

	KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode)));
	dbg(DBG_PRINT,"(GRADING2A 1.a) KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode))) is successful\n");


	if(S_ISCHR(file->vn_mode))
	{
		KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->read);
		dbg(DBG_PRINT,"(GRADING2A 1.a) KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->read) is successful\n");

		read_count = file->vn_cdev->cd_ops->read(file->vn_cdev,offset,buf,count);
		return read_count;
	}
	else
	{
		dbg(DBG_PRINT, "( GRADING2B ) the file is a block device \n");
		return -ENOTSUP;
	}
}
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
    ...
    */
	int write_count;
	KASSERT(file);
	dbg(DBG_PRINT,"(GRADING2A 1.b) KASSERT(file) is successful\n");

	KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode)));
	dbg(DBG_PRINT,"(GRADING2A 1.b) KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode))) is successful\n");


	if(S_ISCHR(file->vn_mode))
	{
		KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->write);
		dbg(DBG_PRINT,"(GRADING2A 1.b) KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->write) is successful\n");

		write_count=file->vn_cdev->cd_ops->write(file->vn_cdev,offset,buf,count);
		return write_count;
	}
	else
	{
