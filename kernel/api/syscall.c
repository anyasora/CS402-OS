...
sys_read(read_args_t *arg)
{
	...
	if ( ( (status = copy_from_user( &rd, arg, sizeof(read_args_t) ) ) < 0) )
	{
		dbg( DBG_PRINT, "\n GRADING3D INSIDE sys_read : ERROR in copy_from_user");
		curthr->kt_errno = status;
		return -1;
	}

	void *tmp_buff = page_alloc();
	if ( (tmp_buff == NULL) )
	{
		dbg( DBG_PRINT, "\n GRADING3D INSIDE sys_read : ERROR in page allocation");
		curthr->kt_errno = ENOMEM;
		return -1;
	}

	int read_bytes = do_read ( rd.fd, tmp_buff, rd.nbytes );
	if ( ( (status = read_bytes) < 0) )
	{
		page_free ( tmp_buff );
		dbg( DBG_PRINT, "\n GRADING3D INSIDE sys_read : ERROR in do_read\n");
		curthr->kt_errno = -status;
		return -1;
	}

	if ( ( (status = copy_to_user ( rd.buf, tmp_buff, read_bytes ) ) < 0) )
	{
		page_free ( tmp_buff );
		dbg( DBG_PRINT, "\n GRADING3D INSIDE sys_read : ERROR in copy_to_user\n");
		curthr->kt_errno = status;
		return -1;
	}

	page_free ( tmp_buff );
	dbg( DBG_PRINT, "\n GRADING3D Exiting sys_read \n");
	return read_bytes;
}
...
sys_write(write_args_t *arg)
{
	...
	if ( ( (status = copy_from_user( &wrt, arg, sizeof(write_args_t) ) ) < 0) )
	{
		dbg( DBG_PRINT, "\n GRADING3D INSIDE sys_write : ERROR in copy_from_user");
		curthr->kt_errno = status;
		return -1;
	}
	...
	if ( (tmp_buff == NULL) )
	{
		dbg( DBG_PRINT, "\n GRADING3D INSIDE sys_write : ERROR in page allocation");
		curthr->kt_errno = ENOMEM;
		return -1;
	}

	if(((status = copy_from_user( tmp_buff, wrt.buf, wrt.nbytes )) < 0))
	{
		dbg( DBG_PRINT, "\n GRADING3D INSIDE sys_write : ERROR in copy_from_user");
		curthr->kt_errno = status;
		return -1;
	...
	int write_bytes = do_write ( wrt.fd, tmp_buff, wrt.nbytes );
	if ( ( (status = write_bytes) < 0) )
	{
		page_free ( tmp_buff );
		dbg( DBG_PRINT, "\n GRADING3D INSIDE sys_read : ERROR in do_write\n");
		curthr->kt_errno = -status;
		return -1;
	}

	page_free ( tmp_buff );
	dbg( DBG_PRINT, "\n GRADING3D EXITING sys_write \n");
	return write_bytes;
}
...
sys_getdents(getdents_args_t *arg)
{
	...
	dbg( DBG_PRINT, "\n GRADING3D ENTERING sys_getdents \n");

	getdents_args_t gd;
	dirent_t tmp_buff;
	int status;

	if ( (status = copy_from_user( &gd, arg, sizeof(getdents_args_t) ) < 0) )
	{
		dbg( DBG_PRINT, "\n GRADING3D INSIDE sys_getdents : ERROR in copy_from_user");
		curthr->kt_errno = status;
		return -1;
	}

	int dents, i=0, max=0, total=0, g=0;

	max = (int) (gd.count / sizeof(dirent_t));

	while ( i < max )
	{
		dbg( DBG_PRINT, "\n GRADING3D INSIDE sys_getdents : Value of i = %d \n", i);
		dents = do_getdent ( gd.fd, &tmp_buff );
		if ( dents < 0 )
		{
			dbg( DBG_PRINT, "\n GRADING3D INSIDE sys_getdents : ERROR in getdent \n");
			curthr->kt_errno = -dents;
			return -1;
		}
		else if ( dents == 0 )
		{
			dbg( DBG_PRINT, "\n GRADING3D INSIDE sys_getdents : getdent = 0 \n");
			break;
		}
		
		if ( ( (status = copy_to_user ( (char*)(gd.dirp) + (sizeof(dirent_t)*g) , &tmp_buff, sizeof(dirent_t) )) < 0) )
		{
			dbg( DBG_PRINT, "\n GRADING3D INSIDE sys_getdents : ERROR in copy_to_user");
			curthr->kt_errno = status;
			return -1;
		}
		total += dents;
	...
	dbg( DBG_PRINT, "\n GRADING3D EXITING sys_getdents \n");
	return total;
}
