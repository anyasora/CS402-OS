...
addr_perm(struct proc *p, const void *vaddr, int perm)
{
	...
	if ( p->p_vmmap )
	{
		dbg( DBG_PRINT, "\n GRADING3D INSIDE addr_perm \n");
		vmarea_t *vma = vmmap_lookup ( p->p_vmmap, ADDR_TO_PN(vaddr) );
		if ( vma && (vma->vma_prot & perm) )
		{
			dbg( DBG_PRINT, "\n GRADING3D addr_perm : Exiting with code  = 1. SUCCESS \n");
			return 1;
		}
	}

	dbg( DBG_PRINT, "\n GRADING3D EXITING addr_perm with code = 0. FAILURE \n");
	return 0;	
}
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
	...
	uint32_t tmp = ADDR_TO_PN(avaddr);
	int code;
	size_t i=0;
	while ( i < len )
	{
		if ( (code = addr_perm (p, avaddr, perm)) )
		{
			dbg( DBG_PRINT, "\n GRADING3D range_perm : calling addr_perm with success return \n");
			tmp++;
			i++;
		}
		else
		{
			dbg( DBG_PRINT, "\n GRADING3D range_perm : calling addr_perm with failure return \n");
			return 0;
		}
	}

	dbg( DBG_PRINT, "\n GRADING3D EXITING range_perm \n");
	return 1;
}
